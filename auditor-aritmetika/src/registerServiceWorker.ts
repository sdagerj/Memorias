 ...
 *
 * Solo en producción: en desarrollo un SW cacheando el bundle estorba mas de lo
...
export function registerServiceWorker(): void {
  ...
  window.addEventListener('load', () => {
    ...
    navigator.serviceWorker.register(url, { scope: './' }).catch((error) => {
      console.info('Service worker no registrado; la app sigue funcionando en línea.', error);
    });
  });
}
